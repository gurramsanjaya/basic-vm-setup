...
class WGExchangeImpl final : public WGExchange::CallbackService {
 ...
 public:
  WGExchangeImpl(std::weak_ptr<ServiceDBusHandler> sys_svc_hdl)
...
class IGrpcHandler {
 protected:
  ...
  IGrpcHandler(std::string url, std::string sys_srvc_nm)
      : url_(std::move(url)),
        svc_(ServiceDBusHandler::get_instance(sys_srvc_nm)) {}
...
int main(int argc, char** argv) {
  ...
  po::options_description desc("Allowed Options", DEFAULT_LINE_LENGTH,
                               DEFAULT_DESCRIPTION_LENGTH);
  desc.add_options()("help", "produce help message")(
      "url", po::value<std::string>()->default_value("127.0.0.1:59910"),
      "server listening endpoint")(
      "wg-conf-file", po::value<std::string>()->default_value("wgs0.conf"),
      "wireguard device conf file in /etc/wireguard/")("tls", "toggle TLS on")(
      "tls-private-key",
      po::value<std::string>()->default_value("./tls/s_private.pem"),
      "private Key File Path")(
      "tls-cert", po::value<std::string>()->default_value("./tls/s_cert.pem"),
      "certificate file path")(
      "timeout", po::value<int>()->default_value(4),
      "timeout value after which the program will shutdown (in seconds)");
  po::variables_map vm;
  ...
  if (vm.count("help") || pos == wg_conf_file.npos ||
      !boost::filesystem::is_regular_file("/etc/wireguard/" + wg_conf_file)) {
    std::cerr << desc << '\n';
    return 1;
  };

  const std::string sys_svc_nm{"wg-quick@" + wg_conf_file.substr(0, pos) +
                               ".service"};
  std::unique_ptr<IGrpcHandler> handler = nullptr;
  if (vm.count("tls")) {
    handler = std::make_unique<TlsGrpcHandler>(
        url, sys_svc_nm, vm["tls-private-key"].as<std::string>(),
