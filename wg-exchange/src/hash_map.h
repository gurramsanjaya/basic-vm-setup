...
template <typename KeyT, typename ValueT, typename HashFn, typename ProbeFn,
          typename EqualityKeyFn = std::equal_to<KeyT>,
          typename AllocatorValueT = std::allocator<ValueT>>
class FixedSizeLockFreeHashMap {
 public:
  static_assert(std::is_copy_assignable<KeyT>::value &&
                    std::is_move_assignable<KeyT>::value,
                "KeyT needs to be copy and move assignable");
  static_assert(std::is_copy_assignable<ValueT>::value &&
                    std::is_move_assignable<ValueT>::value,
                "ValueT needs to be copy and move assignable");
  static_assert(std::is_invocable_r<size_t, HashFn, const KeyT&>::value,
                "Invalid HashFn");
  static_assert(std::is_invocable_r<size_t, ProbeFn, size_t, size_t>::value,
                "Invalid ProbeFn");
  static_assert(
      std::is_invocable_r<bool, EqualityKeyFn, const KeyT&, const KeyT&>::value,
      "Invalid EqualityKeyFn");

  typedef std::pair<const KeyT, ValueT> ValuePair;
  typedef std::optional<ValuePair> OptionalValuePair;

  class Deleter {
    void operator()(FixedSizeLockFreeHashMap* ptr) { destroy(ptr); }
  };
  ...
  class Iterator {
    public:
      ...
      Iterator() = delete;
      Iterator(FixedSizeLockFreeHashMap* m, size_t i, bool w = true)
          : hmap_(m), idx_(i), weak_(w) {}

      // mostly to differentiate end() and invalid()
      // discard weak comparison
      bool operator==(const Iterator& b) { return ((hmap_ == b.hmap_) && idx_ == b.idx_); }
      ...
      reference operator*() { return hmap_->get_value_pair(idx_, weak_); }
      pointer operator->() { return hmap_->get_value_pair(idx_, weak_); }
  ...
  constexpr Iterator invalid() { return Iterator(this, -1); }
 ...
 private:
  ...
  class Track {
    ...
    // sharing.
    alignas(
        std::hardware_destructive_interference_size) std::atomic<Track> track;
  };

  size_t capacity_;
  alignas(
      std::hardware_destructive_interference_size) std::atomic<size_t> size_;
