...
## For ip variation use the following:
# RESULTS_FILE=blocked-ips
# DENY_URLS_FILE=deny_ips
# DENY_EXTRAS=false
# FORCE_ALLOW=false
...
sed -i -E 's/^(http|https|socks4|socks5):\/\/([^:]+)(:[[:digit:]]+){0,1}[[:space:]]*$/\2/' "$RESULTS_TEMP"
